...
type Tree struct {
	...
	Hash stateproofbasics.HashFactory `codec:"hsh"`

	// IsVectorCommitment determines whether the tree was built as a vector commitment
	IsVectorCommitment bool `codec:"vc"`
}

func convertIndexes(elems map[uint64]stateproofbasics.Hashable, treeDepth uint8) (map[uint64]stateproofbasics.Hashable, error) {
	msbIndexedElements := make(map[uint64]stateproofbasics.Hashable, len(elems))
	for i, e := range elems {
...
func hashLeaves(elems map[uint64]stateproofbasics.Hashable, treeDepth uint8, hash hash.Hash) (map[uint64]stateproofbasics.GenericDigest, error) {
	hashedLeaves := make(map[uint64]stateproofbasics.GenericDigest, len(elems))
	for i, element := range elems {
		if i >= (1 << treeDepth) {
			return nil, fmt.Errorf("pos %d >= 1^treeDepth %d: %w", i, 1<<treeDepth, ErrPosOutOfBound)
		}
		hashedLeaves[i] = stateproofbasics.GenericHashObj(hash, element)
	}

	return hashedLeaves, nil
}

func buildFirstPartialLayer(elems map[uint64]stateproofbasics.GenericDigest) partialLayer {
...
func inspectRoot(root stateproofbasics.GenericDigest, pl partialLayer) error {
	computedroot := pl[0]
...
func verifyPath(root stateproofbasics.GenericDigest, proof *Proof, pl partialLayer) error {
	hints := proof.Path
...
// returned by Prove().
func Verify(root stateproofbasics.GenericDigest, elems map[uint64]stateproofbasics.Hashable, proof *Proof) error {
	if proof == nil {
...
func VerifyVectorCommitment(root stateproofbasics.GenericDigest, elems map[uint64]stateproofbasics.Hashable, proof *Proof) error {
	if proof == nil {
		return ErrProofIsNil
	}
